...
#include <wlr/render/fx_renderer.h>
...
static void do_renderer_recreate(void *data) {
	...
	struct fx_renderer *renderer = fx_renderer_create(server->backend);
	if (renderer == NULL) {
	...
	if (allocator == NULL) {
		...
		fx_renderer_destroy(renderer);
		return;
	}

	struct fx_renderer *old_renderer = server->renderer;
	...
	fx_renderer_destroy(old_renderer);
}
